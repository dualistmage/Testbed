...
namespace sf1v5 {
    ...
    int IPRestrictor::parseIPStringToBytes(const std::string& ipAddress, IP_BYTES_TYPE& result) const
    {
        ...
        // Check wrong usage of . in the ipAddress string.
        {
            ...
            while( (pos = ipAddress.find('.', pos)) != std::string::npos )
            {
                if ( (prev_pos + 1) == pos )
                {
                    std::cerr << "[IPRestrictor] Error (line:" << __LINE__ << ") '..' appears in ipAddress : " << ipAddress << std::endl;
                    return false;
                }
                prev_pos = pos;
                pos++;
        ...
        for(tokenizer_type::iterator iter = ipTokenizer.begin(); iter != ipTokenizer.end(); iter++)
        {
            if ( type > 3 )
            {
                std::cerr << "[IPRestrictor] Error (line:" << __LINE__ << ") bad IP format : " << ipAddress<< std::endl;
            ...
            try {
                ...
                {
                    std::cerr << "[IPRestrictor] Error (line:" << __LINE__ << ") bad IP Number : " << *iter << std::endl;
                    return UNKNOWN_IP_TYPE;
                }
    ...
    bool IPRestrictor::addToList(int type, IP_LIST_TYPE& listGroup, IP_BYTES_TYPE& result)
    {
        ...
        {
            std::cerr << "[IPRestrictor] Error (line:" << __LINE__ << ") Wrong Type ip address=" 
                << type << std::endl;
            return false;
        } // end - if
